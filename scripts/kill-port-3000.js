...
async function killPort3000() {
  ...
  try {
    if (os.platform() === 'win32') {
      ...
      try {
        const lsofResult = execSync('lsof -ti:3000', { encoding: 'utf8', stdio: 'pipe' });
      ...
      try {
        const psResult = execSync('ps aux | grep -E "next-server|node.*next" | grep -v grep', { encoding: 'utf8', stdio: 'pipe' });
